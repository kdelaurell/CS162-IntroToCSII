...
* Date: 3/9/16
* Description: Bathroom Source file
**********************************/
...
***************************************************************/
Bathroom::Bathroom(string name, string itemName, string monsterName){
  this->cR1 = NULL;
  this->cR2 = NULL;
  this->cR3 = NULL;
  this->cR4 = NULL;
...
* ran into a monster.
***************************************************************/
void Bathroom::DisplayRoomOptions(){
  int optionCounter = 0;
  if(this->cR1 != NULL){
    optionCounter++;
...
void Bathroom::DisplayItemOptions(){
  cout << "Option 8: Pick up " << this->roomItem << endl;
}

void Bathroom::setConnectedRooms(Spaces* room1, Spaces* room2, Spaces* room3, Spaces* room4, Spaces* room5, Spaces* room6, Spaces* room7){
  this->cR1 = room1;
  this->cR2 = room2;
  this->cR3 = room3;
  this->cR4 = room4;
...
int Bathroom::getNumConnectedRooms(){
  int count = 0;
  if(this->cR1 != NULL)
    count++;
  if(this->cR2 != NULL)
...
string Bathroom::getName(){
  return this->roomName;
}

string Bathroom::getRoomItem(){
  string temp;
  temp = this->roomItem;
  this->roomItem = "";
  return temp;
...
void Bathroom::addItem(string newOldItem){
  this->roomItem = newOldItem;
}

Spaces* Bathroom::getNewLocation(int choice){
  if(choice == 1){
    return this->cR1;
  }
  else if(choice == 2){
...
int Bathroom::monsterAttack(string usedItem){
  return 0;
}

bool Bathroom::hasItem(){
  if(this->roomItem == "")
    return false;
  else
    return true;
...
int Bathroom::intiateSpecial(){
  cout << "You used the restroom. Man that smells!" << endl;
  return 2;
}
